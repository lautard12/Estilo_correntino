...
export async function updateType(id: string, updates: Partial<Pick<ProductType, "name" | "is_active" | "sort_order">>) {
...
export async function createCategory(name: string, type_id: string | null) {
  const { error } = await supabase.from("product_categories").insert({ name, type_id });
  if (error) throw error;
}

export async function updateCategory(id: string, updates: Partial<Pick<ProductCategory, "name" | "type_id" | "is_active" | "sort_order">>) {
...
export async function createVariantSet(name: string) {
  const { data, error } = await supabase.from("variant_sets").insert({ name }).select("id").single();
  if (error) throw error;
  return data.id;
}

export async function updateVariantSet(id: string, updates: Partial<Pick<VariantSet, "name" | "is_active">>) {
...
export async function updateVariantValue(id: string, updates: Partial<Pick<VariantValue, "value" | "is_active" | "sort_order">>) {
  const { error } = await supabase.from("variant_values").update(updates).eq("id", id);
  if (error) throw error;
}

// ─── Price Terms (unified payment options) ───────────────────────────
...
export async function createPriceTerm(term: { code: string; label: string; surcharge_pct: number; default_installments: number | null; fund: string; sort_order: number }) {
  const { error } = await supabase.from("price_terms").insert(term);
  if (error) throw error;
}

export async function updatePriceTerm(id: string, updates: Partial<Omit<PriceTerm, "id">>) {
